...
def mapper():
    ...
    for keys, values in parseInput():
        ...
        if pickup_neighborhood != -1 and dropoff_location != -1:
            if 15 < pickup_time < 20:
                if fare > 0 and tips < fare * 2:
    ...
    for item in pair_agg.keys():
        print '%s\t%s,%.2f,%.2f,%.2f,%.2f,%.2f' % (item, pair_agg[item], duration_agg[item], distance_agg[item], numPass_agg[item], fare_agg[item], tips_agg[item])
